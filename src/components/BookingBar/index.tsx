...
export const BookingBar = () => {
  ...
  // 1. Sync with URL Params on Mount
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        ...
        // Default logic: if only 1 branch, select it; ONLY if not already set by URL
        if (fetchedBranches.length === 1 && !searchParams.get('branch')) {
